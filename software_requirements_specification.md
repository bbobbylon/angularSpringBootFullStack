...
- **Redundant JWT library on the classpath (CON-2).** Both `io.jsonwebtoken:jjwt` (api/impl/jackson) and `com.auth0:java-jwt` are declared, but the code uses `java-jwt` exclusively — `jjwt` has zero imports in `src/`. Removing `jjwt` consolidates the dependency surface and shrinks the CVE-scan footprint; no code change is required.

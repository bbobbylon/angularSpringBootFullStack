...
|---|---|---|---|---|
| **Brand: SecureCapita → TesseraApp** | All five deliverables name the product "SecureCapita" throughout (titles, abstracts, login wireframe, navbar mockup). | Rebranded to **TesseraApp** in `0a2f3ea`; `Grep "SecureCapita"` over `tesseraapp/src/**` returns **0** matches; both SRS docs say TesseraApp. (Submitted docs were already inconsistent — demo accounts use `@tessera.dev` / `TesseraDemo@1`.) | **High** | On any resubmission/reuse, global-replace SecureCapita→TesseraApp (incl. the shield-lock mark + login mockup). The SRS is now the single source of brand truth. |
...
| Frontend API base environment-driven | ✅ | `tesseraapp/src/app/service/{user,customer,admin-user}.service.ts` → `environment.apiUrl` |
...
| Frontend specs | ❌ | `tesseraapp/` | Zero Angular component/service specs. |

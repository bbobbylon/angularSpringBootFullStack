...
| **N** — Public surface, MFA recovery, and access-model closure | 2026-08-09 → 08-15 | Twilio Verify MFA (Verify-service SID owns the code, `TwilioVerifyUtils`); public services catalog endpoint; global footer + Contact Us page + `/features` product-tour page (all three ✅ in §3.5); admin-initiated MFA reset + self-service recovery-code regeneration (§3.1); `cacheInterceptor` test coverage closed (§2.2); `RoleType.isOrganizationScoped(...)` capability check replaces the hardcoded `ROLE_ORGANIZATION_ADMIN` name match in both `AdminUserController` and `AnalyticsController`, closing the `ROLE_HELP_DESK_ADMIN`-unscoped gap (§3.2); `POST /user/verify/resend` redelivers an outstanding 2FA/step-up code without ever minting one from a bare email (FR-AUTH-4); persisted anomaly-signal tuning UI (`securitysettings` singleton row, live-read every login, no redeploy needed) (§3.1); sortable Customers/Invoices list columns (`SortUtils`, allow-listed per entity) (POST-SUBMISSION-UPGRADES.md #1); backend-driven i18n for 403 capability messages via `Accept-Language` + a new `languageInterceptor` (POST-SUBMISSION-UPGRADES.md #2); production boot verified end-to-end under `ddl-auto=validate` against a disposable local database (§2.3); backend-driven HTTP caching (`HttpCacheHeadersFilter`, `Cache-Control: private, no-cache` + ETag) replaces the client-only `cacheInterceptor`/`HttpCacheService`, closing the cross-user staleness gap the old cache could not (§3.4, POST-SUBMISSION-UPGRADES.md #3); `documentation/api-testing/` — parity-checked cURL/Postman/Bruno collections covering all 12 controllers, replacing the stale five-endpoint `APIs.postman_collection` (§3.4, POST-SUBMISSION-UPGRADES.md #4); the new cURL suite's first live run against a running instance caught a real defect in the caching feature it had just shipped — `HttpResponse.timeStamp`'s nanosecond-fresh value busted the ETag hash on every single request, so the `304` short-circuit could never fire — fixed by hashing a Jackson-sanitized copy of the body with that one volatile field stripped, while still serving clients the real, untouched body (POST-SUBMISSION-UPGRADES.md #3 correction); admin User Directory (JDBC) gains sortable columns via a new `SortUtils.resolveSqlOrderBy` allow-listed `ORDER BY` mechanism, extending #1's sorting to the one domain it explicitly left out (POST-SUBMISSION-UPGRADES.md #11); a live-QA pass through the running app (not code review) found and fixed four rendering-layer defects invisible to the existing test suite — login-page footer links wrapping mid-word once a third link was added, a customer/dashboard avatar image escaping its table cell (missing `NgOptimizedImage fill` positioning wrapper), and the footer jumping to the navbar during route transitions (`<main>` needed `flex: 1` to hold its own height while the route-transition animation absolutely-positions the outgoing/incoming views) — plus one reported "Manage Services" gap investigated and found to already be fully implemented (POST-SUBMISSION-UPGRADES.md #12); a `user.events.total` Micrometer counter tagged by `EventType` added at `NewUserEventListener`'s single event-listener seam, giving every audit event (login success/failure, suspicious-login step-up, MFA/passkey enrollment, token reuse, …) a live real-time metric alongside its existing DB-audit-row, exposed at `/actuator/metrics` behind the same `UPDATE:USER`/`UPDATE:ROLE` authority `/admin/**` requires (`/actuator/health`/`/info` stay public for the ALB/ECS health check) — closing a lockstep gap the same fix exposed, where `Constants.PUBLIC_ROUTES`' bare `/actuator` entry would have skipped JWT parsing on `/actuator/metrics` and turned the new gate into a permanent 403 for everyone instead of an admin-only allow; `logback-spring.xml` added (none existed before), switching `prod`/`qa`/`stage` console output to one JSON object per line via `logstash-logback-encoder` while leaving `dev`/`local`'s human-readable console untouched, catching and fixing a `CONSOLE_LOG_PATTERN_IS_UNDEFINED` regression (Boot's `defaults.xml` needs an unconditional top-level include, not one nested inside a `springProfile` branch) before it shipped (POST-SUBMISSION-UPGRADES.md #5) |

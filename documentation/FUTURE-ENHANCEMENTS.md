...
| ⬜ **Real SMS delivery** | `NotificationServiceImpl.java:54` — SMS 2FA sends only when Twilio credentials are set, and is otherwise a documented no-op. Honest, but one advertised factor is not exercisable | Either wire Twilio properly or drop SMS from the factor list. A factor that silently does nothing is worse than one that does not exist |

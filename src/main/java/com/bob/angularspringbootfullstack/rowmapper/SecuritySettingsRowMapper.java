 ...
 *
 * <p>{@code anomaly_enabled}, {@code anomaly_history_limit}, and {@code max_concurrent_sessions} all
 * use {@link ResultSet#getObject(String, Class)} rather than {@code getBoolean}/{@code getInt},
 * which is deliberate: those two return {@code false} and {@code 0} for a SQL {@code NULL} instead
 * of a Java {@code null}, silently turning "no override on record" into "override to disabled" /
 * "override to zero" — exactly the values this table exists to distinguish from "unset".
 */
public class SecuritySettingsRowMapper implements RowMapper<SecuritySettings> {
    ...
    public SecuritySettings mapRow(ResultSet resultSet, int rowNum) throws SQLException {
        ...
        return SecuritySettings.builder()
                ...
                .maxConcurrentSessions(resultSet.getObject("max_concurrent_sessions", Integer.class))
